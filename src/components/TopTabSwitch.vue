...
function getBadgeClass(tabId: string) {
  if (props.tone === 'default') {
    return props.modelValue === tabId ? 'bg-foreground text-background' : 'bg-muted text-muted-foreground';
  }
...
watch(() => props.modelValue, syncMeasurements);
watch(() => props.tabs.map((tab) => `${tab.id}:${tab.label}:${tab.badge ?? ''}`).join('|'), syncMeasurements);
watch(() => props.collapseInactive, syncMeasurements);
  ...
  <div class="flex items-center gap-0.5" role="tablist" :aria-label="props.ariaLabel">
    ...
    >
      ...
      <span
        ...
        class="absolute left-[7.5px] top-1/2 z-10 flex w-[17px] -translate-y-1/2 items-center justify-center"
      >
        <i
          :class="[
            tab.icon,
