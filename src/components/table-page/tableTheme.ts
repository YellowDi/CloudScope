...
export const tableTheme = {
  wrapper:
    'relative min-w-0 w-full max-w-full overflow-hidden rounded-[calc(var(--radius)+4px)] border border-border-whisper bg-card shadow-[var(--shadow-card)]',
  scrollViewport:
  ...
  row: 'group transition-colors hover:bg-surface-hover-subtle',
  summary: 'border-t border-border px-4 py-3 text-[13px] text-muted-foreground',
  headerCell: {
    base: 'h-[38px] border-b border-border bg-surface-secondary px-3 py-0 text-left align-middle text-[13px] font-medium whitespace-nowrap',
  },
  bodyCell: {
